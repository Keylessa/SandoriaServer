...
std::vector<CharacterData> CharacterDatabase::getCharacters(const std::string& accountName) {
    ...
    try {
        std::unique_ptr<sql::PreparedStatement> pstmt(conn->prepareStatement(
            "SELECT name, faction, class, race, gender, level FROM characters WHERE account = ?"));

        pstmt->setString(1, accountName);
        std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());

        while (res->next()) {
            characters.push_back({ res->getString("Name"), res->getString("Level") });
        }
    }
    catch (sql::SQLException& e) {
        std::cerr << "[MySQL Error] " << e.what() << std::endl;
...
std::vector<CharacterData> CharacterDatabase::getCharacters(const std::string& accountName) {
    ...
    try {
        // Adăugăm și coloana "guid" și "account" pentru a avea mai multe informații
        std::unique_ptr<sql::PreparedStatement> pstmt(conn->prepareStatement(
            "SELECT guid, account, name, faction, class, race, gender, level "
            "FROM characters WHERE account = ?"
        ));
